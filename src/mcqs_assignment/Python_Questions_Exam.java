 ...
 * @author rames
 */
public class Python_Questions_Exam extends JFrame implements ActionListener {
    ...
    public Python_Questions_Exam(){
        ...
        lblques=  new JLabel("Welcome to the Python examination. Please Click Start button to begin the test.");
    ...
    {  
        ...
        try
        {
            ...
            if(current==k)  
            {    
                String sql="select * from science_questions order by rand()"; //selects all fields of table 'science_questions' 
    ...
    {  
        ...
        {
            Statement stmt = User_Registration_Form.getConnection().createStatement();
            String sq="select * from science_questions";
    ...
    void showAnswerKey() //function to print answer key if requested for
    {
        try
        {
            ...
            for(int i=1;i<=10;i++)
            {
                String sq="select Correct_ans from science_questions where ID="+i+"";
                ResultSet rs2 = stmt.executeQuery(sq);
                rs2.next();
