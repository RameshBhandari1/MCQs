...
public class Java_Exams_Questions extends JFrame implements ActionListener{
    ...
    public Java_Exams_Questions(){
        ...
        lblques=  new JLabel("Welcome to the Java examination. Please Click Start button to begin the test.");
    ...
    {  
        ...
        try
        {
            ...
            if(current==k)  
            {    
                String sql="select * from java_questions order by rand()"; //selects all fields of table 'java_questions' 
    ...
    {  
        ...
        {
            Statement stmt = User_Registration_Form.getConnection().createStatement();
            String sq="select * from java_questions";
    ...
    void showAnswerKey() //function to print answer key if requested for
    {
        try
        {
            ...
            for(int i=1;i<=10;i++)
            {
                String sq="select Correct_ans from java_questions where ID="+i+"";
                ResultSet rs2 = stmt.executeQuery(sq);
                rs2.next();
